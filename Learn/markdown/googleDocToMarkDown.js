/*
Note: If you have anchor links to headings, add a Table Of Contents (linked) in the begining of document
...
function handleToc(elem) {
  var numTOCChildren = elem.getNumChildren();
      for (var j = 0; j < numTOCChildren; j++) {
        TOCChild = elem.getChild(j);
        if (TOCChild.getType() == DocumentApp.ElementType.PARAGRAPH ||
            TOCChild.getType() == DocumentApp.ElementType.LIST_ITEM) {
          var headingText = TOCChild.getText();
          if (headingText) {
            headingTextLink = headingText.toLowerCase().replace(/ /g, '-');
            TOCLinkURL = TOCChild.getLinkUrl();
            if (TOCLinkURL) {
              internalUrlsMap[TOCLinkURL] = '#'+headingTextLink;
            }       
          }
        }
      }
  ...
  if (element.getType() === DocumentApp.ElementType.TABLE) {
    ...
    for (var i = 0; i < element.getNumChildren(); i++) {
      ...
      for (var j = 0; j < nCols; j++) {
        ...
        var tableCellParagraph = tableCell.findElement(DocumentApp.ElementType.PARAGRAPH).getElement();
        var tableCellText = tableCellParagraph.findElement(DocumentApp.ElementType.TEXT).getElement();
        textElements.push("<td>" );
        textElements.push(tableCellText);
        textElements.push(' </td>\n ' );
      }
      textElements.push("  </tr>\n");
    }
...
function processTextElement(inSrc, txt) {
  ...
  for (var i=attrs.length-1; i>=0; i--) {
    ...
    if (url) {  // start of link
      ...
      if (internalUrlsMap[url]) {
        url = internalUrlsMap[url];
      }
